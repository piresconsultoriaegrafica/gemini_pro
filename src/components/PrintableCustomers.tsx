...
export const PrintableCustomers = React.forwardRef<HTMLDivElement, PrintableCustomersProps>(({ customers, companyInfo }, ref) => {
  ...
  return (
    <div ref={ref} className="p-4 font-sans text-slate-800 bg-white print:p-2">
      ...
      <h1 className="text-sm font-bold uppercase text-center mb-3 tracking-widest text-indigo-900 bg-indigo-50 py-1.5 rounded-xl print:break-inside-avoid">LISTA DE CLIENTES</h1>
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-indigo-50 text-indigo-900">
            ...
            <th className="border-b border-indigo-100 p-1.5 text-left text-[8px] uppercase tracking-wider">Telefone</th>
            <th className="border-b border-indigo-100 p-1.5 text-left text-[8px] uppercase tracking-wider">Email</th>
            <th className="border-b border-indigo-100 p-1.5 text-left text-[8px] uppercase tracking-wider rounded-tr-xl">Documento</th>
          </tr>
        </thead>
        <tbody>
          {customers.map((c, index) => (
            <tr key={c.id} className={`text-[8px] ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
              <td className="border-b border-slate-100 p-1.5 text-slate-800 font-medium">{c.name}</td>
              <td className="border-b border-slate-100 p-1.5 text-slate-600">{c.phone || '-'}</td>
              <td className="border-b border-slate-100 p-1.5 text-slate-600">{c.email || '-'}</td>
              <td className="border-b border-slate-100 p-1.5 text-slate-600">{c.document || '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
